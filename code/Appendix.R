...
# Following the final project, here is a version of it using a close-up of Montreal
# This project uses a map with pixels of 7 x 7 m
...
# Bird observation dataset sources: 
  # Auer T, Barker S, Barry J, Charnoky M, Curtis J, Davies I, Davis C, Downie I, Fink 
  # D, Fredericks T, Ganger J, Gerbracht J, Hanks C, Hochachka W, Iliff M, Imani J, 
  # Jordan A, Levatich T, Ligocki S, Long M T, Morris W, Morrow S, Oldham L, Padilla 
  # Obregon F, Robinson O, Rodewald A, Ruiz-Gutierrez V, Schloss M, Smith A, Smith J, 
  # Stillman A, Strimas-Mackey M, Sullivan B, Weber D, Wolf H, Wood C (2024). EOD – eBird 
  # Observation Dataset. Cornell Lab of Ornithology. Occurrence dataset https://doi.org/10
  # .15468/aomfnb accessed via GBIF.org on 2025-02-12.
  # iNaturalist contributors, iNaturalist (2025). iNaturalist Research-grade Observations. 
  # iNaturalist.org. Occurrence dataset https://doi.org/10.15468/ab3s5x accessed via GBIF.org 
  # on 2025-02-12.
...
# Montreal satellite image dataset source : 
  ...
  # (https://browser.dataspace.copernicus.eu/?zoom=13&lat=45.48541&lng=-73.61458&themeId
  # =DEFAULT-THEME&visualizationUrl=U2FsdGVkX19V5TuOrMZ0evgtMJ4khCgYGBKdCueloWJ5o3mMkGve
  # paGHNmelNKjjzFvvWTfri8xvcWZq295OrUf5MtDvKLDg1wfPBMjOIvLad9PvMQJtdSZKa2PJ9QeP&dataset
  # Id=S2_L2A_CDAS&fromTime=2018-06-15T00%3A00%3A00.000Z&toTime=2018-06-15T23%3A59%3A59.
  ...
  # Parameters of download : 
    ...
    # Coordinate system : WGS 84 (EPSG:4326)
      # Resolution:
      # lat.: 0.0000630 deg/px (0.2sec/px)
      # long.: 0.0000898 deg/px (0.3sec/px)
...
# Calling the GBIF dataset
  birds <- read.table("occurrence2.txt", header=TRUE, sep="\t", fill = TRUE)
...
# Loading the satellite images of Montreal as rasters
  mtlB8 <- rast("Montreal2018_TEST_B8.tiff") # This is the NIR band
  mtlB4 <- rast("Montreal2018_TEST_B4.tiff") # This is the red band
...
# Testing if the data is normally distributed
  ...
  # -0.2931632 --> The data is lightly skewed to the left (NOT normally distributed)
  
  # Measuring the taildness
  kurtosis(df_BN$birds2)
  # -0.9934651 --> The data has a light tail and is flatter than a normal distribution
...
# Result and interpretation of the Wilcoxon Rank-Sum test
  ...
  # W = 7.173e+10 (the test statistic value)
...
# Result of the Spearman Rank Correlation interpretation
  ...
  # and the NDVI clusters, as expected. The rho values is still small so the relationship 
  # is weak. 
